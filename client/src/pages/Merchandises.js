...
const Merchandises = () => {
  ...
  const getMerchandises = async (page = 1) => {
    let res = await axios.get(`/api/merchandises?page=${page}`)
    setCurrentPage(page)
    console.log('total pages', res.data)
    setTotalPages(res.data.total_pages)
    createSellerArray(res.data.merchandises)
  }
  ...
  const createSellerArray = (data) =>{
    ...
    let sellerArray = []
    ids.map( id => {
      let merchandises = data.filter( d => d.seller_id === id );
      let { seller_id, name, email } = merchandises[0];

      let sellerMerchandises = merchandises.map( m => { 
        let { description, price, category, merch_id } = m;
        return { description, price, category, merch_id };
      });

      let detail = { seller_id, name, email, merchandises: sellerMerchandises, };
  ...
  const renderMerchandises = (merchandises) => {
    // console.log(merchandises)
    return merchandises.map( m => 
  ...
  const renderSellers = () =>{
    ...
    // console.log(sellers)
      return (
        <List>
          { sellers.map( seller => {
              let { seller_id, name, email, merchandises } = seller;
              return (
                <List.Item key={seller_id}>
                  ...
                  <List.Item>
                    <Table celled>
                      ...
                      <Table.Body>
                        {renderMerchandises(merchandises)}
                      </Table.Body>
                    </Table>
                  </List.Item>
                </List.Item>
